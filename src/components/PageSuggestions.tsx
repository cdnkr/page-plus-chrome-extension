  ...
  return (
    <div className="w-full p-4 pb-[200px]">
      <div className="text-sm text-black/80 dark:text-white/80 mb-3">
        {t('pageSuggestions.suggestionsFor')} <span className="font-medium text-black dark:text-white">{currentPageUrl.length > 22 ? currentPageUrl.slice(0, 22) + '...' : removeTrailingSlash(currentPageUrl)}</span>
      </div>
      <div className="space-y-2">
        {suggestions.map((suggestion, index) => (
          <button
            ...
            className={cn(
              "w-full relative text-left px-3 py-4 rounded-[20px] bg-black/5 dark:bg-white/5 cursor-pointer",
          ...
          >
            <div className="font-base text-sm text-gray-900 dark:text-gray-200 mb-1 max-w-[90%] text-wrap">
              {suggestion.title}
            </div>
